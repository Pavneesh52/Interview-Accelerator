...
class Interview(Base):
    ...
    status: Mapped[InterviewStatus] = mapped_column(SQLEnum(InterviewStatus, values_callable=lambda x: [str(e.value) for e in x]), default=InterviewStatus.NOT_STARTED, nullable=False)
    current_level: Mapped[InterviewLevel] = mapped_column(SQLEnum(InterviewLevel, values_callable=lambda x: [str(e.value) for e in x]), default=InterviewLevel.SCREENING, nullable=False)
...
class InterviewQuestion(Base):
    ...
    level: Mapped[InterviewLevel] = mapped_column(SQLEnum(InterviewLevel, values_callable=lambda x: [str(e.value) for e in x]), nullable=False)
    question_text: Mapped[str] = mapped_column(Text, nullable=False)
    question_type: Mapped[QuestionType] = mapped_column(SQLEnum(QuestionType, values_callable=lambda x: [str(e.value) for e in x]), nullable=False)
    expected_competencies: Mapped[list] = mapped_column(JSON, nullable=False, default=list)
    difficulty: Mapped[DifficultyLevel] = mapped_column(SQLEnum(DifficultyLevel, values_callable=lambda x: [str(e.value) for e in x]), default=DifficultyLevel.MEDIUM, nullable=False)
